...
using IsContainerCompatible = std::enable_if_t<IsContainerCompatibleImpl<
    ContainerRef, std::remove_cvref_t<ContainerRef>, ElementType>>;
...
public:
    ...
    {
        assert(
            (offset <= size_ && (count == dynamic_extent || offset + count <= size_)) &&
            "Sliced range not in array.");
        return {data_ + offset, count == dynamic_extent ? size_ - offset : count};
    }
    ...
    {
        assert(idx < size() && "Index out or range");
        return data_[idx];
    }
...
public:
    ...
    {
        assert(
            (Offset <= size_ && (Count == dynamic_extent || Count <= size_ - Offset)) &&
            "Sliced range not in array.");
        return {data_ + Offset, Count == dynamic_extent ? size_ - Offset : Count};
    }
    ...
    {
        assert(offset <= size_ && "Cannot slice past end of array.");
    ...
    {
        assert(n <= size_ && "Cannot remove such a long prefix");
        data_ += n;
        size_ -= n;
    }
    ...
    {
        assert(n <= size_ && "Cannot remove such a long suffix");
        size_ -= n;
    }
...
struct tuple_size<gt::span<ElementType, Extent>> : integral_constant<size_t, Extent>
{};

template<typename ElementType>
struct tuple_size<gt::span<ElementType, gt::dynamic_extent>>; // not defined

template<size_t Index, typename ElementType, size_t Extent>
struct tuple_element<Index, gt::span<ElementType, Extent>>
{
...
struct tuple_element<Index, gt::span<ElementType, gt::dynamic_extent>>; // not defined

template<size_t Index, typename ElementType, size_t Extent>
[[nodiscard]] constexpr ElementType& get(gt::span<ElementType, Extent> span) noexcept
{
    static_assert(Index < Extent, "span index out of bounds");
    return span[Index];
}
...
get(gt::span<ElementType, gt::dynamic_extent> span) noexcept = delete;
